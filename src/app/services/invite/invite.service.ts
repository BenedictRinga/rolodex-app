...
export class InviteService {
  ...
  /** 2026-08-18 SHAREAPP: the share text carries the OG landing URL so the
   *  link preview itself is the branded card; it redirects into the PWA. */
  shareUrl(inv: RolodexInvite): string {
    return inv?.ogUrl || inv?.url || `https://zyppar.com/rolodex/?invite=${inv.token}`;
  }
  ...
  async share(inv: RolodexInvite, text?: string): Promise<'shared' | 'copied' | 'failed'> {
    ...
    const fallback = inv.kind === 'appointment'
      ? `${inv.from} fixed a meeting with you on RolodexAI: ${inv.title || 'An appointment'}${inv.when ? ' — ' + new Date(inv.when).toLocaleString() : ''}. ${url}`
      : `${inv.from} sent you a message on RolodexAI: "${inv.text}". ${url}`;
    const finalText = text || fallback;
    const nav = navigator as any;
    try {
      if (nav.share) {
        await nav.share({ title: 'RolodexAI invite from ' + inv.from, text: finalText, url });
        return 'shared';
      }
