...
// 2026-08-29 BUILD 142: the ONE language state — every selector now reads and
...
export class LoopInboxComponent implements OnInit, OnDestroy {
  ...
  /** 2026-08-29 BUILD 142: a LIVE getter, not a snapshot — the select always
   *  shows the real current language, whatever surface set it last. */
  get currentLang(): string {
    return this.translate.currentLang || this.translation.getSavedLanguage() || 'en';
  }
  ...
  /** 2026-08-29 BUILD 142: delegate to the ONE language service - persists the
   *  choice, applies the user's translation overrides, and every other
   *  surface (Settings, next launch) sees the same language instantly. */
