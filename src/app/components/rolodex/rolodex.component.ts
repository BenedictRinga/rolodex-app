...
export class RolodexComponent implements OnInit {
   ...
   *  2026-08-29 BUILD 142: a LIVE getter, not a snapshot — whatever surface set
   *  the language last, this select shows the truth. */
  readonly languages = APP_LANGUAGES;
  get currentLang(): string {
    return this.translate.currentLang || this.translation.getSavedLanguage() || 'en';
  }

  /** 2026-08-29 BUILD 142: delegate to the ONE language service - persists,
   *  applies overrides, and the Inbox select mirrors it instantly. */
  ...
  async cycleShareVoice(ev?: Event): Promise<void> {
    ...
    const order: Array<'auto' | 'A' | 'B' | 'C' | 'D' | 'E' | 'F'> = ['auto', 'A', 'B', 'C', 'D', 'E', 'F'];
